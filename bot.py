...
DAILY_BONUS_STARS = Decimal("0.5")  # Fixed to 0.5 stars
...
TEXTS = {
    "ps": {
        "btn_withdraw": "🏧 ویډرا",
        "btn_mystars": "⭐ زما ستوري",
        "btn_referral": "👥 ریفرل",
        "btn_tasks": "📝 تاسکونه",
        "btn_bonus": "🎁 بونس",
        "btn_lang": "🌐 ژبه",
        "btn_about": "ℹ️ زمونږ په اړه",
        "btn_support": "📞 سپورټ",
        "btn_admin_stats": "📊 Statistics",
        "btn_admin_broadcast": "📣 Broadcast",
        "btn_admin_add_task": "🛠 Add Task",
        "btn_admin_rem_task": "🗑 Remove Task",
        "btn_admin_add_bal": "➕ Add Balance",
    ...
    "fa": {
        "btn_withdraw": "🏧 برداشت",
        "btn_mystars": "⭐ ستاره‌های من",
        "btn_referral": "👥 زیرمجموعه",
        "btn_tasks": "📝 وظایف",
        "btn_bonus": "🎁 پاداش",
        "btn_lang": "🌐 زبان",
        ...
        "intro": "به ربات EasyEarn خوش آمدید",
        "force_join": "لطفاً ابتدا در کانال‌های زیر عضو شوید:",
        "joined_btn": "✅ عضو شدم",
        "join_failed": "لطفاً ابتدا در تمام کانال‌های مورد نیاز عضو شوید.",
        "my_stars": "⭐ ستاره‌های شما: {stars}",
        "referral": "👥 لینک زیرمجموعه‌گیری شما:\n{link}\n\nشما 15% از درآمد تأییدشده زیرمجموعه‌های خود را دریافت می‌کنید.\nتعداد زیرمجموعه‌ها: {count}",
        "tasks_empty": "❌ در حال حاضر هیچ وظیفه‌ای موجود نیست.",
        "task_done": "✅ وظیفه انجام شد\n⭐ {stars}",
        "task_already": "شما قبلاً این وظیفه را انجام داده‌اید.",
        "task_fail": "❌ ابتدا در کانال/گروه عضو شوید، سپس تأیید کنید.",
        "task_bot_fail": "❌ تأیید خودکار این وظیفه ممکن نیست. لطفاً عکس صفحه ارسال کنید.",
        "bonus_added": "✅ پاداش روزانه دریافت شد: {stars} ⭐",
        "bonus_wait": "⏳ پاداش قبلاً دریافت شده است. زمان باقی‌مانده: {remaining}",
        "withdraw_choose": "💸 روش برداشت را انتخاب کنید:",
        "withdraw_low": "❌ موجودی کافی نیست.",
        "withdraw_cooldown": "⏳ برداشت در حال حاضر قفل است. زمان باقی‌مانده: {remaining}",
        "admin_low": "❌ موجودی مدیر کم است.",
        "about": "ℹ️ درباره ما\n\nربات EasyEarn سیستمی برای کسب ستاره از طریق وظایف، زیرمجموعه‌گیری و پاداش روزانه است.",
        "support": "📞 پشتیبانی\n\nلطفاً پیام دهید:\n{username}",
        "new_task": "📢 وظیفه جدید اضافه شد!\n\n{title}\n⭐ پاداش: {reward}",
        "stats_admin": "👥 کل کاربران: {users}\n🆕 کاربران امروز: {today}\n⭐ کل ستاره‌های کاربران: {stars}\n⭐ ستاره‌های مدیر: {admin_stars}\n📝 وظایف فعال: {tasks}",
        "admin_only": "این بخش فقط برای مدیر است.",
        "admin_help": "🛠 Admin Commands\n\n/users\n/refstats\n/withdraws\n/botstats\n/taskslist\n/taskstats\n/ban USER_ID [reason]\n/unban USER_ID",
        "broadcast_prompt": "پیامی که می‌خواهید برای همه ارسال شود را بفرستید.",
        "addtask_kind": "نوع وظیفه را انتخاب کنید:",
        "addtask_link": "لینک کانال/گروه یا @username را بفرستید.",
        "addtask_title": "عنوان وظیفه را بفرستید.",
        "addtask_reward": "مقدار پاداش را بفرستید، مثال: 0.5",
        "addtask_post_link": "لینک پست را بفرستید.",
        "addtask_bot_link": "لینک ربات را بفرستید. مثال: https://t.me/SomeBot?start=abc",
        "addbalance_prompt": "تعداد ستاره‌هایی که می‌خواهید به حساب مدیر اضافه کنید را بفرستید. مثال: 1000",
        "addbalance_done": "✅ موجودی مدیر {amount} ستاره افزایش یافت.\n⭐ موجودی جدید: {new_balance}",
        "removetask_prompt": "یک وظیفه را برای حذف انتخاب کنید.",
        "cancelled": "❌ عملیات لغو شد.",
        "open_task_btn": "🔗 باز کردن وظیفه",
        "verify_btn": "✅ تأیید",
        "send_proof_btn": "📸 ارسال اثبات",
        "task_item": "📢 {title}\n⭐ پاداش: {stars}",
        "leave_notice": "⚠️ شما یک کانال/گروه پاداش‌دار را ترک کردید. پاداش شما کسر شد و وظیفه دوباره فعال شد.",
        "new_withdraw": "📤 درخواست برداشت جدید!",
        "proof_prompt": "📸 لطفاً همین الان عکس اثبات را بفرستید.",
        "proof_saved": "✅ اثبات دریافت شد. در انتظار بررسی مدیر.",
        "proof_rejected": "❌ اثبات شما رد شد. وظیفه دوباره در دسترس است.",
        "proof_approved": "✅ اثبات شما تأیید شد.\n⭐ {stars}",
        "banned": "⛔ حساب شما مسدود شده است. برای راهنمایی با پشتیبانی تماس بگیرید.",
        "all_tasks_done": "✅ شما تمام وظایف موجود را انجام داده‌اید",
        "withdraw_sent": "✅ درخواست برداشت شما ثبت شد.\n⏳ ستاره‌های شما تا ۱۲ ساعت آینده ارسال خواهد شد.",
        "withdraw_support_no_username": "⚠️ شما نام کاربری ندارید. برای برداشت با پشتیبانی نیز تماس بگیرید: {username}",
...
def get_text(lang: str, key: str, **kwargs) -> str:
    ...
    txt = d.get(key, TEXTS["en"].get(key, key))
    return txt.format(**kwargs)


def resolve_menu_action(text: str) -> Optional[str]:
    """Resolve multilingual button clicks into actions"""
    for lang, strings in TEXTS.items():
        for key, val in strings.items():
            if key.startswith("btn_") and text == val:
                return key
    # Fallback to English checking strictly if some keyboards mismatch locally
    for key, val in TEXTS["en"].items():
        if key.startswith("btn_") and text == val:
            return key
    return None
...
def init_db():
    ...
    safe_exec("ALTER TABLE promo_chats ADD COLUMN IF NOT EXISTS msg_count INTEGER DEFAULT 0")
...
def main_menu(user_id: int) -> ReplyKeyboardMarkup:
    keyboard = [
        [t(user_id, "btn_withdraw"), t(user_id, "btn_mystars")],
        [t(user_id, "btn_referral"), t(user_id, "btn_tasks")],
        [t(user_id, "btn_bonus"), t(user_id, "btn_lang")],
        [t(user_id, "btn_about"), t(user_id, "btn_support")],
    ]
    if user_id == ADMIN_ID:
        keyboard.insert(0, [t(user_id, "btn_admin_stats"), t(user_id, "btn_admin_broadcast")])
        keyboard.insert(1, [t(user_id, "btn_admin_add_task"), t(user_id, "btn_admin_rem_task")])
        keyboard.insert(2, [t(user_id, "btn_admin_add_bal")])
...
def single_task_keyboard(user_id: int, task: dict, page: int = 0) -> InlineKeyboardMarkup:
    ...
    rows.append([InlineKeyboardButton(back_text, callback_data=f"tasks_page_{page}")])
    return InlineKeyboardMarkup(rows)
...
async def channel_post_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    row = fetch_one("SELECT is_active, COALESCE(msg_count, 0) as msg_count FROM promo_chats WHERE chat_id = %s", (chat.id,))
    if row and row.get("is_active") == 1:
        count = row["msg_count"] + 1
        if count >= 10:
            try:
                await context.bot.send_message(chat_id=chat.id, text=PROMO_TEXT)
                execute("UPDATE promo_chats SET msg_count = 0 WHERE chat_id = %s", (chat.id,))
            except Exception as exc:
                logger.info("promo failed for %s: %s", chat.id, exc)
        else:
            execute("UPDATE promo_chats SET msg_count = %s WHERE chat_id = %s", (count, chat.id))
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "check_force_join":
        if await check_force_join_all(context.bot, user.id):
            ...
            if lang == "fa":
                ...
                msg = "✅ Access granted."
            await query.message.reply_text(msg, reply_markup=main_menu(user.id))
        else:
    ...
    if data.startswith("verify_"):
        ...
        if not joined:
            if reason == "bot_not_admin":
                lang = get_lang(user.id)
                msg = "❌ Bot is not admin in the channel/group."
                if lang == "ps":
                    msg = "❌ بوټ په چینل/ګروپ کې اډمین نه دی."
                elif lang == "fa":
                    msg = "❌ ربات در کانال/گروه مدیر نیست."
                await query.message.reply_text(msg, reply_markup=main_menu(user.id))
            else:
    ...
    if data.startswith("proof_") and not data.startswith("proof_ok_") and not data.startswith("proof_no_"):
        if not await guard_user_access(update, context):
            return
        _, task_id, page = data.split("_")
        task = get_task(int(task_id))
    ...
    if data.startswith("proof_ok_"):
        ...
        execute(
            "UPDATE user_tasks SET admin_review_message_id = %s, last_checked_at = %s WHERE id = %s AND status = 'completed'",
            (query.message.message_id, now_iso(), record_id),
        )
        
        try:
            await query.message.edit_reply_markup(reply_markup=None)
            await query.message.edit_caption(caption=build_proof_review_caption(row, approved=True))
        except Exception:
            try:
    ...
    if data.startswith("proof_no_"):
        ...
        try:
            await query.message.edit_reply_markup(reply_markup=None)
            await query.message.edit_caption(caption=build_proof_review_caption(row, approved=False))
        except Exception:
            try:
...
async def admin_flow_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if text.lower() in ("cancel", "/cancel", "❌ cancel", "back", "⬅️ back", "❌ لغوه", "❌ انصراف", "بازگشت", "⬅️ بازگشت", "⬅️ شاته"):
        for key in [
            "admin_flow",
            "new_task_type",
            "task_chat_username",
